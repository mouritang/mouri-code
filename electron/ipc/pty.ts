...
export function spawnAgent(
  ...
  // Clear env vars that can leak parent-agent session context into child
  // sessions. Without this, nested Codex/Claude runs may print internal
  // tool-call payloads (e.g. "assistant to=... tool_uses ...") in the PTY.
  delete spawnEnv.CLAUDECODE;
  delete spawnEnv.CLAUDE_CODE_SESSION;
  delete spawnEnv.CLAUDE_CODE_ENTRYPOINT;
  delete spawnEnv.CODEX_THREAD_ID;
  delete spawnEnv.CODEX_INTERNAL_ORIGINATOR_OVERRIDE;
  const sanitizedSpawnEnv = Object.fromEntries(
    Object.entries(spawnEnv).filter(([key]) => !key.startsWith('CODEX_INTERNAL_')),
  );
...
export function getAgentMeta(agentId: string): {
  ...
  return s ? { taskId: s.taskId, agentId: s.agentId, kind: s.kind, command: s.command } : null;
}
