...
function buildGrid(panelId: string): string[][] {
  ...
  if (task) {
    const grid: string[][] = [['title'], ['shell-toolbar']];
...
export function getTaskFocusedPanel(taskId: string): string {
  const current = store.focusedPanel[taskId];
  if (!current) return defaultPanelFor(taskId);
  const grid = buildGrid(taskId);
  return findInGrid(grid, current) ? current : defaultPanelFor(taskId);
}
