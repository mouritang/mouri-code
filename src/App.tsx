...
function App() {
  ...
  onMount(async () => {
    ...
    await loadAgents();
    await loadState();
    await syncGlobalMonitorConfig();
    await refreshGlobalMonitorStatus();
    ...
    const unlistenCloseRequested = await appWindow.onCloseRequested(async (event) => {
      ...
      try {
        const countLabel =
          runningCount === 1 ? '1 个正在运行的终端会话' : `${runningCount} 个正在运行的终端会话`;
        const shouldKill = await confirm(
          `当前有 ${countLabel}。重启应用后可恢复。要结束它们并退出，还是让它们在后台继续运行？`,
          {
            title: '终端仍在运行',
            kind: 'warning',
            okLabel: '结束并退出',
