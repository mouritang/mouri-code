...
export function TaskPanel(props: TaskPanelProps) {
  ...
  const [showBranchDialog, setShowBranchDialog] = createSignal(false);
  ...
  // Auto-focus AI terminal when task first becomes active (if no panel set yet)
  createEffect(() => {
    if (props.isActive && !store.focusedPanel[props.task.id]) {
      const id = props.task.id;
      setTimeout(() => {
        // Only focus AI terminal if no panel was set in the meantime
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '4px', 'margin-left': '8px', 'flex-shrink': '0' }}>
            ...
            <IconButton
              icon={
                <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                  <path d="M3.75 2A1.75 1.75 0 0 0 2 3.75v8.5C2 13.216 2.784 14 3.75 14h8.5A1.75 1.75 0 0 0 14 12.25v-5a.75.75 0 0 0-1.5 0v5a.25.25 0 0 1-.25.25h-8.5a.25.25 0 0 1-.25-.25v-8.5a.25.25 0 0 1 .25-.25h5a.75.75 0 0 0 0-1.5h-5Zm9.78.22a.75.75 0 0 1 0 1.06L8.56 8.25a.75.75 0 0 1-1.06 0L5.97 6.72a.75.75 0 1 1 1.06-1.06l1 1 4.44-4.44a.75.75 0 0 1 1.06 0Z" />
                </svg>
              }
                    ...
                    return (
                      ...
                      >
                        ...
                        <Show when={shellExits[shellId]}>
                          ...
                          >
                            进程已退出（{shellExits[shellId]?.exitCode ?? '?'}）
                          </div>
                        </Show>
                        <TerminalView
    ...
    return {
      ...
      content: () => (
        <ScalablePanel panelId={`${props.task.id}:ai-terminal`}>
          ...
          >
            <InfoBar title={props.task.lastPrompt || '使用终端内置输入框与智能体对话'}>
              <span style={{ opacity: props.task.lastPrompt ? 1 : 0.4 }}>
                {props.task.lastPrompt ? `> ${props.task.lastPrompt}` : '在终端内直接输入并发送'}
              </span>
            </InfoBar>
  ...
  return (
    ...
    >
      <Show when={props.task.closingStatus && props.task.closingStatus !== 'removing'}>
        ...
        >
          <Show when={props.task.closingStatus === 'closing'}>
            <div style={{ 'font-size': '13px', color: theme.fgMuted }}>正在关闭任务...</div>
          </Show>
      ...
      <ResizablePanel
        ...
        children={[titleBar(), branchInfoBar(), shellSection(), aiTerminal()]}
      />
      <TaskDialogs
        task={props.task}
        ...
        onBranchDialogDone={() => setShowBranchDialog(false)}
        showCommitDialog={showCommitDialog()}
        onCommitDialogDone={() => setShowCommitDialog(false)}
        diffFile={diffFile()}
